...
const searchTreat = async (req, res) => {
  ...
  if(!mongoose.Types.ObjectId.isValid(id)){
    return res.status(400).json({ error: "Treat not found!" });
  }
...
const createTreat = async (req, res) => {
  const { title, description, host, status } = req.body;

  try {
    const treat = await Treat.create({ title, description, host, status });
    res.status(200).json(treat);
  } catch (error) {
